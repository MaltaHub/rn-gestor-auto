...
-- 1. First, update existing lojas to have proper tenant_id references
-- For now, we'll set all existing lojas to the first tenant
-- In production, this would need to be done more carefully
UPDATE lojas 
SET tenant_id = (SELECT id FROM tenants LIMIT 1)
WHERE tenant_id IS NULL;

-- 2. Make tenant_id NOT NULL after fixing existing data
...
-- 4. Update view_veiculos_expandidos to include tenant information
...
-- 5. Create proper RLS policies for veiculos table
DROP POLICY IF EXISTS "Users can view veiculos of their tenant" ON veiculos;
...
-- 6. Create RLS policies for veiculos_loja
...
-- 7. Create function to auto-assign users to tenants based on email domain
...
-- 8. Add unique constraint to prevent duplicate memberships
